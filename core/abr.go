...
func (c *ApiPosterConf) NewPoster() (poster *apiPoster, err error) {
	...
	if c.SrcFileColumSeparator == "" {
		poster.SrcFileColumSeparator = ColSeparatorDefault
	}
	...
	if poster.ApiPosterConf.reqHeader, err = ParseReqHeader(poster.ApiPosterConf.Header); err != nil {
		err = errors.New("parse conf headers fail: " + err.Error())
		return
	}
...
func (c *apiPoster) itemPost(item string, client *http.Client) (err error, resData string) {
	...
	if reflect.TypeOf(param).Kind() == reflect.String {
		...
		if datajs, err = sonic.Marshal(param); err != nil {
			return errors.New("serialize req body err: " + err.Error()), ""
		}
	}
	...
	if c.BuiltInParamAppender != nil {
		...
		if err = json.Unmarshal(datajs, &dataMap); err != nil {
			fmt.Printf("json Unmarshal req err: " + err.Error() + string(datajs))
			return fmt.Errorf("json Unmarshal req err: %s", err.Error()), ""
		}
	...
	if c.reqHeader != nil {
		CompleteReqHeader(req.Header, c.reqHeader, item, c.SrcFileColumSeparator)
	}
	...
	if c.DryRun {
		fmt.Println(item, "\n", "[body] "+string(datajs), "\n", "[header] ", HeadersToDebugString(req.Header))
		return
	}
	...
	for i := 0; i < len(template); i++ {
		if template[i] == '$' {
			...
			if i+1 < len(template) && template[i+1] >= '0' && template[i+1] <= '9' {
				...
				// 检查索引是否有效
				if num > len(parts) {
					return "", fmt.Errorf("index %d out of range for line, line col num %d ", num, len(parts))
				}
				// 替换占位符
				var writeStr string
				if num == 0 {
